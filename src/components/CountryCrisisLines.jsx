...
export function CountryCrisisLines() {
  ...
  return (
    <div className={`rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5 transition-all ${isSkyMode ? 'glass-card-day' : 'glass-card-night'}`}>
      ...
      <p className={`text-xs sm:text-sm ${isSkyMode ? 'text-bluey-700' : 'text-bluey-300'}`}>
      ...
      <select
        ...
        className={`w-full sm:w-64 px-4 py-2.5 rounded-xl text-sm ${isSkyMode ? 'input-day' : 'input-night'}`}
      >
      ...
      <div className="space-y-2.5">
        {selected.lines.map((line, idx) => (
          <div
            key={idx}
            className={`p-3.5 rounded-2xl border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 ${isSkyMode ? 'bg-white/60 border-bluey-200' : 'bg-bluey-900/50 border-bluey-700'}`}
          >
            <div>
              <div className={`font-bold text-sm ${isSkyMode ? 'text-bluey-950' : 'text-white'}`}>{line.name}</div>
              {(line.hours || line.note) && (
                <div className={`text-[11px] ${isSkyMode ? 'text-bluey-600' : 'text-bluey-400'}`}>
            ...
            <div className="flex items-center gap-4 text-sm font-semibold">
              {line.phone && (
                <a
                  ...
                  className={`flex items-center gap-1.5 ${isSkyMode ? 'text-bluey-700 hover:text-bluey-900' : 'text-seafoam-300 hover:text-seafoam-200'}`}
                >
                  <Phone className="w-3.5 h-3.5" /> {line.phone}
                </a>
              )}
              {line.text && (
                <span className={`flex items-center gap-1.5 ${isSkyMode ? 'text-bluey-700' : 'text-seafoam-300'}`}>
      ...
      <a
        ...
        className={`text-xs font-semibold flex items-center gap-1.5 hover:underline w-fit ${isSkyMode ? 'text-bluey-700' : 'text-bluey-300'}`}
      >

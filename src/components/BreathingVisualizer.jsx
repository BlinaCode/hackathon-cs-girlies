...
export function BreathingVisualizer() {
  ...
  const goPrevMode = () => {
    if (isActive) return;
    const nextIdx = (modeIndex - 1 + modeKeys.length) % modeKeys.length;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-10 px-2 sm:px-0">
      ...
      <div className="text-center space-y-1.5">
        ...
        >
          <Compass
            className={`w-6 h-6 ${isSkyMode ? "text-bluey-500" : "text-seafoam-400"}`}
          />
          Ocean Wave Breathing
        </h2>
        <p
          className={`text-xs sm:text-sm max-w-md mx-auto font-medium ${isSkyMode ? "text-bluey-700" : "text-bluey-300"}`}
        >
          Synchronize your breath with expanding ocean waves to calm your
          nervous system.
        </p>
      </div>
      ...
      {participants.length > 0 && (
        <div className="flex items-center justify-center gap-2">
          ...
          >
            <span
              className={`w-2 h-2 rounded-full animate-pulse ${isSkyMode ? "bg-bluey-400" : "bg-seafoam-400"}`}
            />
      ...
      {/* Sisu Mascot Speech */}
      <OtterMascot
        expression={isActive ? "breathing" : "caring"}
        speech={
          isActive
            ? "Breathe with me..."
      ...
      <div className="flex items-center justify-center gap-2">
        {modeKeys.map((key, idx) => (
          <div
            ...
            style={{
              width: idx === modeIndex ? "2rem" : "0.5rem",
              background:
                idx === modeIndex
                  ? isSkyMode
                    ? "#A4D3DE"
      ...
      <div
        ...
        style={
          isSkyMode
            ? { background: `${accent}33`, borderColor: accent }
            : { background: `${accent}26`, borderColor: `${accent}88` }
        }
      >
        ...
        {/* Technique name + how-to descriptor */}
        <div className="text-center space-y-1 relative z-10">
          <h3
            className={`font-display text-lg sm:text-xl font-bold ${isSkyMode ? "" : "text-slate-100"}`}
            style={isSkyMode ? { color: "#4A2511" } : undefined}
          >
            {mode.name}
          </h3>
          <p
            className={`text-xs sm:text-sm max-w-sm mx-auto font-medium ${isSkyMode ? "" : "text-slate-400"}`}
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-6 relative z-10">
          ...
          <div className="relative w-56 h-56 sm:w-72 sm:h-72 flex items-center justify-center shrink-0">
                ...
                isSkyMode
                  ? currentPhase.name === "Inhale"
                    ? {
                        ...
                        borderColor: "#A4D3DE",
                        boxShadow: "inset 0 4px 16px rgba(255,255,255,0.7)",
                      }
                    : currentPhase.name === "Exhale"
                      ? {
                          background: "rgba(245,232,201,0.5)",
                          borderColor: "#E3CE9E",
                        }
                      : { background: "#FAFBF0", borderColor: "#E8E2D1" }
                  : currentPhase.name === "Inhale"
                    ? {
                        background: "rgba(164,211,222,0.18)",
                        borderColor: "#5FA0AF",
                        boxShadow: "inset 0 2px 14px rgba(255,255,255,0.15)",
                      }
                    : currentPhase.name === "Exhale"
                      ? {
            ...
            >
              <span
                className={`text-xs uppercase font-bold tracking-widest mb-1 ${isSkyMode ? "" : "text-bluey-300"}`}
                style={isSkyMode ? { color: "#7A5A3A" } : undefined}
              >
                {currentPhase.name}
              </span>
              <span
                className={`font-display text-4xl sm:text-5xl font-bold ${isSkyMode ? "" : "text-slate-100"}`}
                style={isSkyMode ? { color: "#4A2511" } : undefined}
              >
                {secondsRemaining}s
              </span>
              <span
                className={`text-xs mt-[13px] font-bold ${isSkyMode ? "" : "text-slate-400"}`}
                style={isSkyMode ? { color: "#7AABB8" } : undefined}
              >
            ...
            <button
              ...
              style={
                isSkyMode
                  ...
                  : {
                      background: "rgba(74,158,101,0.18)",
                      borderColor: "rgba(74,158,101,0.4)",
                      color: "#8FD9A8",
                    }
              }
            >
        ...
        <p
          ...
          style={isSkyMode ? { color: "#7A5A3A" } : undefined}
        >
          Use the arrows to browse techniques — Start whenever you're ready.
        </p>
      </div>
    </div>
  );
